...
sim.QLfit <- function(p.beta, i.beta, e.beta, S, L, U){
  ...
  res <- list(pvalue.covmodel = result.fit2$P.values[[3]],
              qvalue.covmodel = result.fit2$Q.values[[3]],
              pvalue.nocovmodel = result.fit$P.values[[3]],
              qvalue.nocovmodel = result.fit$Q.values[[3]],
              beta.ind = beta.ind, 
              m1.estimate.nocovmodel = round(nrow(counts) - result.fit$m0),
              m1.estimate.covmodel = round(nrow(counts) - result.fit2$m0),
              mean.count = apply(counts, 2, mean),
              counts=counts,
              x = c(x[1,], x[2,]),
              S = S, 
              U= U, 
...
# set.seed(1)
# sim <- sim.counts(p.beta[5], 1.5, 2, S[1], 2, 3)
# sim <- sim.counts(p.beta[5], 1.5, 2, S[1], 1.5,2)
